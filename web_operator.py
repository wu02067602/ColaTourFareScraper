...
class WebNavigator:
    ...
    def login_to_website(self, username: str, password: str, captcha_model_path: str) -> None:
        ...
        try:
            ...
            screenshot_handler.capture_and_upload(self.driver, "find_element_error_imgValidate")
            raise RuntimeError(f"找不到驗證碼圖片元素: {e}")
        # 取得 image_element 的完整 HTML，用於除錯
        image_html = image_element.get_attribute("outerHTML")
        print(f"驗證碼圖片元素的 HTML: {image_html}")
